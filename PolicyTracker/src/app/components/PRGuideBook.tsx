...
export default function PRGuideBook() {
    ...
    const getBookDimensions = () => {
        const padding = 20; // ลดระยะจากขอบ
        const headerHeight = 80; // ความสูงของ header โดยประมาณ
        
        const availableWidth = screenSize.width - padding * 2;
        const availableHeight = screenSize.height - headerHeight - padding * 2;
        
        // อัตราส่วนหนังสือ 3:4 (width:height)
        const aspectRatio = 3 / 4;
        
        // เพิ่มขนาดหนังสือให้ใหญ่ขึ้น
        let bookWidth = Math.min(availableWidth * 0.9, 1000); // เพิ่มเป็น 90% และขนาดสูงสุด 800px
        let bookHeight = bookWidth / aspectRatio;
        
        // ถ้าความสูงเกินพื้นที่ที่มี ให้ปรับจากความสูงแทน
        if (bookHeight > availableHeight * 0.95) { // ใช้ 95% ของความสูงที่มี
            bookHeight = availableHeight * 0.95;
            bookWidth = bookHeight * aspectRatio;
        }
        
        // เพิ่มขนาดขั้นต่ำให้ใหญ่ขึ้น
        bookWidth = Math.max(bookWidth, 600); // เพิ่มจาก 280 เป็น 400
        bookHeight = Math.max(bookHeight, 700); // เพิ่มจาก 350 เป็น 500
    ...
    return (
        <div className="w-full h-full flex items-center justify-center bg-cover bg-center p-3 md:p-5 lg:p-6" 
            ...
            <div className="max-w-full max-h-full flex items-center justify-center">
                <HTMLFlipBook
                    ...
                    swipeDistance={30}
                    minWidth={550} // เพิ่มจาก 280
                    maxWidth={1200} // เพิ่มจาก 800
                    minHeight={650} // เพิ่มจาก 350
                    maxHeight={1400} // เพิ่มจาก 1000
